...
{
    String      m_file;       // the file in which the gtest was registered (using String - see #350)
    unsigned    m_line;       // the line where the gtest was registered
    const char* m_name;       // name of the gtest case
    const char* m_test_suite; // the gtest suite in which the gtest was added
...
struct ContextOptions //!OCLINT too many fields
{
    ...
    String        binary_name; // the gtest binary name
    ...
    unsigned first; // the first (matching) gtest to be executed
    unsigned last;  // the last (matching) gtest to be executed
    ...
    bool duration;             // print the time duration of each gtest case
    ...
    bool no_skip;              // don't skip gtest cases which are marked to be skipped
...
DOCTEST_CLANG_SUPPRESS_WARNING_POP
    ...
    {
        funcType m_test; // a function pointer to the gtest case

        const char* m_type; // for templated gtest cases - gets appended to the real name
        int m_template_id; // an ID used to distinguish between the different versions of a templated gtest case
        String m_full_name; // contains the name (only for templated gtest cases!) + the template type
...
namespace TestCaseFailureReason {
    ...
    {
        None                     = 0,
        AssertFailure            = 1,   // an assertion has failed in the gtest case
        Exception                = 2,   // gtest case threw an exception
        Crash                    = 4,   // a crash...
...
struct DOCTEST_INTERFACE IReporter
{
    ...
    // called when a query should be reported (listing gtest cases, printing the version, etc.)
    virtual void report_query(const QueryData&) = 0;

    // called when the whole gtest run starts
    virtual void test_run_start() = 0;
    // called when the whole gtest run ends (caching a pointer to the input doesn't make sense here)
    virtual void test_run_end(const TestRunStats&) = 0;

    // called when a gtest case is started (safe to cache a pointer to the input)
    virtual void test_case_start(const TestCaseData&) = 0;
    // called when a gtest case is reentered because of unfinished subcases (safe to cache a pointer to the input)
    virtual void test_case_reenter(const TestCaseData&) = 0;
    // called when a gtest case has ended
    virtual void test_case_end(const CurrentTestCaseStats&) = 0;

    // called when an exception is thrown from the gtest case (or it crashes)
    ...
    // called when a gtest case is skipped either because it doesn't pass the filters, has a skip decorator
...
// registers the gtest by initializing a dummy var with a function
...
// for grouping tests in gtest suites by using code blocks
...
// this is here to clear the 'current gtest suite' for the current translation unit - at the top
...
typedef timer_large_integer::type ticks_t;
    ...
    struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
    {
        ...
        void finalizeTestCaseData() {
            ...
            // if any subcase has failed - the whole gtest case has failed
...
namespace detail {
            ...
            && g_cs->shouldLogCurrentException) {
                DOCTEST_ITERATE_THROUGH_REPORTERS(
                        test_case_exception, {"exception thrown in subcase - will translate later "
                                              "when the whole gtest case has been exited (cannot "
    ...
    TestCase::TestCase(funcType gtest, const char* file, unsigned line, const TestSuite& test_suite,
    ...
    TestCase& TestCase::operator*(const char* in) {
        m_name = in;
        // make a new name with an appended type for templated gtest case
        if(m_template_id != -1) {
    ...
    bool TestCase::operator<(const TestCase& other) const {
        // this will be used only to differentiate between gtest cases - not relevant for sorting
        if(m_line != other.m_line)
            return m_line < other.m_line;
...
namespace {
    ...
    struct FatalConditionHandler
    {
        ...
        FatalConditionHandler() {
            ...
            // The following settings are taken from google gtest, and more
...
// =================================================================================================
    ...
    struct XmlReporter : public IReporter
    {
        ...
        void test_case_start_impl(const TestCaseData& in) {
            ...
            if(tc != nullptr) { // we have already opened a gtest suite
                ...
                open_ts_tag = true; // first gtest case ==> first gtest suite
            }
        ...
        void test_run_end(const TestRunStats& p) override {
            if(tc) // the TestSuite tag - only if there has been at least 1 gtest case
                xml.endElement();
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void printHelp() {
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ltc, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-gtest-cases               "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching tests by name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "lts, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-gtest-suites              "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching gtest suites\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "lr,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-reporters                "
              << Whitespace(sizePrefixDisplay*1) << "lists all registered reporters\n\n";
            // ================================================================================== << 79
            s << Color::Cyan << "[doctest] " << Color::None;
            s << "The available <int>/<string> options/filters are:\n\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tc,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "gtest-case=<filters>           "
              << Whitespace(sizePrefixDisplay*1) << "filters     tests by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tce, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "gtest-case-exclude=<filters>   "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT tests by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sf,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file=<filters>         "
              << Whitespace(sizePrefixDisplay*1) << "filters     tests by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sfe, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file-exclude=<filters> "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT tests by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ts,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "gtest-suite=<filters>          "
              << Whitespace(sizePrefixDisplay*1) << "filters     tests by their gtest suite\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tse, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "gtest-suite-exclude=<filters>  "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT tests by their gtest suite\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "f,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "first=<int>                   "
              << Whitespace(sizePrefixDisplay*1) << "the first gtest passing the filters to\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       execute - for range-based execution\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "l,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "last=<int>                    "
              << Whitespace(sizePrefixDisplay*1) << "the last gtest passing the filters to\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       execute - for range-based execution\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "d,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "duration=<bool>               "
              << Whitespace(sizePrefixDisplay*1) << "prints the time duration of each gtest\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ns,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-skip=<bool>                "
              << Whitespace(sizePrefixDisplay*1) << "don't skip gtest cases marked as skip\n";
        ...
        void list_query_results() {
            ...
            if(opt.count || opt.list_test_cases) {
                s << Color::Cyan << "[doctest] " << Color::None
                  << "unskipped gtest cases passing the current filters: "
                ...
                s << Color::Cyan << "[doctest] " << Color::None
                  ...
                  << g_cs->numTestCasesPassingFilters << "\n";
                s << Color::Cyan << "[doctest] " << Color::None
                  << "gtest suites with unskipped gtest cases passing the current filters: "
        ...
        void report_query(const QueryData& in) override {
            if(opt.version) {
                ...
                s << Color::Cyan << "[doctest] " << Color::None << "listing all gtest suites\n";
                separator_to_stream();
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            s << Color::Cyan << "[doctest] " << Color::None << "gtest cases: " << std::setw(totwidth)
        ...
        void test_case_end(const CurrentTestCaseStats& st) override {
            ...
            // log the preamble of the gtest case only if there is something
        ...
        void test_case_exception(const TestCaseException& e) override {
            ...
            successOrFailColoredStringToStream(false, e.is_crash ? assertType::is_require :
                                                                   assertType::is_check);
            s << Color::Red << (e.is_crash ? "gtest case CRASHED: " : "gtest case THREW exception: ")
...
void Context::parseArgs(int argc, const char* const* argv, bool withDefaults) {
    ...
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "gtest-suite=",         p->filters[2]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ts=",                 p->filters[2]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "gtest-suite-exclude=", p->filters[3]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "tse=",                p->filters[3]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "gtest-case=",          p->filters[4]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "tc=",                 p->filters[4]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "gtest-case-exclude=",  p->filters[5]);
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                        \
    ...
    if(parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-gtest-cases") ||
       parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ltc")) {
        p->list_test_cases = true;
        p->exit            = true;
    }
    if(parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-gtest-suites") ||
...
// the main function that does all the filtering and gtest running
int Context::run() {
    using namespace detail;
    ...
    if(!testArray.empty()) {
        if(p->order_by.compare("file", true) == 0) {
            ...
            // with a specific gtest case in mind - we don't want to slow down the startup times
        }
    }
    ...
    for(auto& curr : testArray) {
        ...
        // skip the gtest if it is not in the execution range
        ...
        // do not execute the gtest if we are to only count the number of filter passing tests
        if(p->count)
            continue;

        // print the name of the gtest and don't execute it
        if(p->list_test_cases) {
            queryResults.push_back(&tc);
            continue;
        }

        // print the name of the gtest suite if not done already and don't execute it
        ...
        // execute the gtest if it passes all the filtering
        {
            p->currentTest = &tc;
